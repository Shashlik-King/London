...
def getInfo(df):
    ...
    clustering_vector = df['clustering_division_mult_D'].dropna().values.tolist()
    clustering_vector=[x*float(df['Diameter'][0]) + (float(df['Diameter'][0]/2)) for x in clustering_vector]
    
    circle_division = df['quarter_circle_divisions'].dropna().values.tolist()
    
    vertical_separation = df['vertical_separation'].dropna().values.tolist()
    
    sPropList=[]
    soil_layers=[0]
    for i in range(len(df['t_i'].dropna().values.tolist())):
        
        sPropList.append(dict(t=float(df['t_i'].dropna().values.tolist()[i]),Constitutive=str(df['Constitutive_model'].dropna().values.tolist()[i]),Eoed=float(df['E_oed'].dropna().values.tolist()[i]),
    ...
    return pPropDict, seaLevel, forceDict, DispDict, sPropList, soil_layers, pile_thickness, thickness_layers, global_scour, local_scour, clustering_vector, circle_division, vertical_separation