function [kttop ktbot] = tanspringstiff(settings,element,pile,loads,y_topbottom,i)
%--------------------------------------------------------------------------
% PURPOSE
...
if settings.Apply_Direct_springs && ~ strcmp(element.model_py(i),'Zero soil')
   PLAXIS=settings.Plaxis_data.PYCurves;
    [kttop ktbot] =Direct_tan(PLAXIS,element,pile,y_topbottom/element.degradation_py_y(i),i);
else 
