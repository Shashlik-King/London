...
for n = 1:settings.n_max                    % Looping over load increments
    ...
    for j = 1:settings.j_max                % N-R Iteration procedure
		...
		if i<=(nelem-1) %only pile nodes         
		   [kstop ksbot]       = secspringstiff(settings,element,pile,loads,y_topbottom,i); % secant stiffness for distributed lateral load 
            if settings.mteta
				[ksmomtop ksmombot] = secmomstiff(settings,element,pile,teta_topbottom,y_topbottom,kstop,ksbot,i);
			else 
				ksmomtop = 0; 
				ksmombot = 0; 
			end
		elseif settings.toe_shear %only the extra node for base element
            [kstop ksbot]       = secHBstiff(settings,element,pile,y_topbottom,i); % secant stiffness for base shear load  
            [ksmomtop ksmombot] = secMBstiff(settings,element,pile,teta_topbottom,i);  % secant stiffness for base moment
  ...
  if settings.mteta   
           ...
           if i<=(nelem-1) %only pile nodes   
            [kttop ktbot]       = tanspringstiff(settings,element,pile,loads,y_topbottom,i);
			if settings.mteta
				[ktmomtop ktmombot] = tanmomstiff(settings,element,pile,teta_topbottom,y_topbottom,kstop,ksbot,i);  % tangent stiffness for distributed moment
			else 
				ktmomtop = 0; 
				ktmombot = 0; 
			end
           ...
           elseif settings.toe_shear %only the extra node for base element        
            [kttop ktbot]       = tanHBstiff(settings,element,pile,y_topbottom,i); % tangent stiffness for base shear load  
            [ktmomtop ktmombot] = tanMBstiff(settings,element,pile,teta_topbottom,i);  % tangent stiffness for base moment
           else
            kttop = 0; 
            ktbot = 0; 
            ktmomtop = 0; 
     ...
     if settings.mteta
        ...
        elseif j == settings.j_max && plots.Inverse == 1
            warning([' No convergence in step ' num2str(n) ' - increment ' num2str(j)])
            output.stop = 1;
            break
%         end
