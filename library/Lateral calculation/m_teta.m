...
%--------------------------------------------------------------------------
for c = 1:(nelem-1)
    teta_top(c,:) = [0 0.0000003 0.000001 0.000003 0.00001 0.00003 0.0001 0.0003 0.001 0.01 0.1 0.2 0.3];
    teta_bot(c,:) = teta_top(c,:);
    upy_top(c,:) = [0 0.0001 0.001 0.005 0.003 0.12 0.4 ];
    upy_bot(c,:) = upy_top(c,:);    
    ...
    for j = 1:npoints2
		[ksppynode_top ksppynode_bot] = secspringstiff(settings,element,pile,loads,[upy_top(c,j) upy_bot(c,j)],c);
        ppynode_top(c,j) = ksppynode_top*upy_top(c,j);  
        ppynode_bot(c,j) = ksppynode_bot*upy_bot(c,j); 
		for k=1:npoints
			[ksmtetanode_top ksmtetanode_bot] = secmomstiff(settings,element,pile,[teta_top(c,k) teta_bot(c,k)],[upy_top(c,j) upy_bot(c,j)],ksppynode_top,ksppynode_bot,c);
    ...
    %                 output.ts = toe_shear(element,node,pile,ybot,c);
               [kMb_top(j) kMb_bot(j)] = secMBstiff(settings,element,pile,[tetabot(j) tetabot(j)],g); 
