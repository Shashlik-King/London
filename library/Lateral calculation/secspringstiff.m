function [kstop ksbot] = secspringstiff(settings,element,pile,loads,y_topbottom,i)
% 
%--------------------------------------------------------------------------
% PURPOSE
...
if settings.Apply_Direct_springs && ~ strcmp(element.model_py(i),'Zero soil')
   ...
   PLAXIS=settings.Plaxis_data.PYCurves ;
    [kstop ksbot] =Direct_Sec(PLAXIS,element,pile,y_topbottom/element.degradation_py_y(i),i);
else 
