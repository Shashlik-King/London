...
ID = Location;			 
for loc = 1:size(ID,1)
    clearvars -except loc ID model pile_length_LC pile_length_end load_case object_layers variable PLAX PYcreator loadcase scour soil pile loads settings weight PYcreator_stiff NStepMultiplier var_name constant con_name Database Apply_Direct_springs txt_file_output
    plaxis_file_name = 'Structural_Forces_D_10.xlsx';
...
data.revision.output        = 2;  % revision no. for storing results into the database  
...
settings.j_max                  = 200;      % max. no. of iterations in a load step (lateral only)
settings.TOL                    = 1e-4;     % relative tolerance (lateral only)
settings.n_max                  = 500;       % number of load steps (lateral only)
%--------------------------------------------------------------------------
...
if plots.load_deflection == 1 
	settings.max_load_ratio     = NStepMultiplier;        % FKMV changed from 250 attempts to apply a load equal to the one imported from Excel/database multiplied by this factor
end
%--------------------------------------------------------------------------
%% Output files
%--------------------------------------------------------------------------
...
elseif settings.rel_error == 1 && plots.load_deflection == 1
    [results] = PISA_rel_error_load_disp(weight,Es,element,output,PLAX);
end
...
if txt_file_output
    ...
    output_COPCAT.output.load_def = output ;
