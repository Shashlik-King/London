...
mysqlstr_ini        = ['INSERT INTO ',settings.db_table,' (Unit_name,Cons_name,Project_name,Rev,Layer_number,Depth,Pile_length,Pile_Diameter,'...
...
%     end
    ...
    for j = 1:length(index)
        ...
        database_str = [mysqlstr_ini,'''',b{1},''',''',a{1},''',''',strcat(New.project{index(j)},'_',num2str(j)),''',',num2str(New.revision),',',c{1},',',num2str(element.level(j,1)),',',num2str(pile.(char(Input.ModelsSpil(Geo))).length_start(1)),',',num2str(pile.(char(Input.ModelsSpil(Geo))).diameter(1)),''];
        for ii = 1:16
